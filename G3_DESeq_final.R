...
# cellobiose vs glucose 
g3res_cel_glu <- results(g3dds, contrast=c("groupName","cellobiose","glucose"))
g3res_cel_glu
g3resmat_cel_glu <- as.matrix(g3res_cel_glu)
g3resmat_cel_glu
colnames(g3resmat_cel_glu) <- paste("cellobiose_v_glucose", colnames(g3resmat_cel_glu), sep = "_")
...
all(rownames(g3resmat_FP_cel) == rownames(g3resmat_cel_glu))
...
CBA_DESeq <- cbind(CBA_DESeq, g3resmat_cel_glu)
CBA_DESeq <- CBA_DESeq[, c(1:15,17,21)]
CBA_DESeq$glucose_v_cellobiose_Sig <- ifelse(CBA_DESeq$cellobiose_v_glucose_padj <= 0.05,"TRUE","FALSE")

CBA_DESeq$GeneID <- rownames(CBA_DESeq)
CBA_DESeq <- CBA_DESeq[,c(19,1:18)]
  
library(writexl)
write_xlsx(CBA_DESeq, "/Users/colleenahern/Documents/EBTNG3/Finalized_codes/Tables/CBA_DESeq_final.xlsx")
