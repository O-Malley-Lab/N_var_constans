# 11/15/2023
# # Post G3 DESeq analysis: Plot supplement heatmaps
...
# order p-values
vary_43 <- vary_43[order(vary_43$cellobiose_glucose_Padj,decreasing=FALSE),]
colnames(vary_43) <- c("geneID", "Cellobiose", "cellobiose_glucose_Padj", "FP", "FP_glucose_Padj", "RCG", "RCG_glucose_Padj")
...
# replace the JGI name of any CAZyme with the dBcane name
...
fullcazgenelist <- fullcazgenelist[,c(2,1,10,11,12,3:9)]
...
fullcazgenelist <- fullcazgenelist[,c(1,2,13,6:11)]
#fullcazgenelist <- fullcazgenelist[order(fullcazgenelist$cellobiose_glucose_Padj,decreasing=FALSE),]
#fullcazgenelist <- fullcazgenelist[order(fullcazgenelist$FP_glucose_Padj,decreasing=FALSE),]
fullcazgenelist <- fullcazgenelist[order(fullcazgenelist$RCG_glucose_Padj,decreasing=FALSE),]
